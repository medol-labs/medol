...
- `for each Source.collection as item`: Declares event automation fan-out. The automation emits one command per collection item, and command fields may map from `item.field`.
...
- `id`: Marks the field as an identity field for command routing, events, read models, or concept selection.
- `generated`: Marks a command field whose value is generated by the application instead of entered by the user.
- `technical`: Marks metadata or implementation-facing fields that are still part of the model contract.
- `query`: Marks a read/query access field.
- `display`: Marks the preferred human-readable label field for generated UI selectors.
- `uploadFile`: Marks a command field that carries browser file upload content to a generated multipart endpoint.
- `file`: Marks a command field that stores a staged file reference selected by the UI.
- `portOutput`: Marks an event field produced by an external port adapter result rather than copied from the command or state.

Use `portOutput` on event fields when a `port` slice delegates work to an adapter and the adapter creates a value that must be recorded in the resulting event. For example, a runtime engine adapter may create `runtimeEngineJobId` only after submitting a job. That field should not be forced into the triggering command as an empty placeholder; model it as:
...
Code generators should expose `runtimeEngineJobId` on the port success result and write the event from `portResult.runtimeEngineJobId`.
