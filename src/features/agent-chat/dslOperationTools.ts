...
const parseTarget = (target: string): { kind: string; name: string } | undefined => {
  ...
  const match = /^(domain|context|aggregate|constraint|slice|command|event|error|readmodel|projection|automation|policy|integration|specification|hotspot)\s+(.+)$/i.exec(normalized);
  if (!match) return undefined;
  return {
