...
export const generatePrd = (model: EmModel, options: PrdGenerateOptions = {}): PrdGenerationResult => {
  ...
  const title = humanize(model.domains[0]?.name ?? context?.name ?? 'Product Requirements');
  const documentId = `prd:${toDslId(model.domains[0]?.name ?? context?.name ?? 'product')}`;
...
const buildPrdDocument = (model: EmModel, id: string, title: string): PrdDocument => {
  ...
  const slices = contexts.flatMap((context) =>
    context.aggregates.flatMap((aggregate) =>
      aggregate.slices.map((slice) => toPrdSlice(slice, aggregate.name))
    )
  );
  ...
  return {
    ...
    context: primaryContext?.name ?? 'EventModel',
...
const buildOverview = (model: EmModel, context: EmContext | undefined): string => {
  const domainName = model.domains[0]?.name;
  const contextName = context?.name ?? 'the modeled context';
  if (domainName) {
    return `${humanize(domainName)} describes the ${humanize(contextName)} product area through event-modeled business capabilities, state changes, policies, read models, and integrations.`;
  }
  return `${humanize(contextName)} is described through event-modeled business capabilities, state changes, policies, read models, and integrations.`;
};

const toPrdSlice = (slice: EmSlice, aggregate: string): PrdSlice => {
...
const collectOpenQuestions = (model: EmModel, slices: PrdSlice[]): string[] => {
  ...
  for (const slice of slices) {
    ...
    if (!slice.businessRules.length && slice.command) {
      questions.push(`Add acceptance criteria for ${humanize(slice.name)}.`);
    }
  }
  return questions;
};
