...
export const localizeDocumentationMarkdown = (
  ...
  const replacements: Array<[string, string]> = [
    ...
    ['Collection', '集合/列表'],
    ['Single record', '单条记录'],
    ['Not explicitly modeled', 'DSL 中未明确'],
    ['No UI views are explicitly modeled.', 'DSL 中未明确页面视图。'],
    ['No structural gaps were detected.', '未检测到结构性缺口。'],
    ['No process risks or open questions are explicitly modeled.', 'DSL 中未明确流程风险或待确认事项。'],
    ['The DSL contains diagnostics. Treat affected sections as incomplete until these are resolved.', 'DSL 存在诊断错误；相关章节在问题修复前应视为不完整。']
  ];
